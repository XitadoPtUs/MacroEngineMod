...
class TextEditor {
    ...
    fun keyPressed(keyCode: Int, font: Font, clipboard: String): Boolean {
        ...
        when {
            Screen.hasControlDown() && keyCode == InputConstants.KEY_A -> {
            ...
            Screen.hasControlDown() && keyCode == InputConstants.KEY_V -> {
                insert(clipboard)
                return true
            }
    ...
    private fun drawHighlightedLine(
        ...
        for (span in highlight.spans) {
            ...
            graphics.drawString(font, MacroFonts.text(segment), drawX, lineY, span.color, false)
    ...
    private fun moveCursor(left: Boolean) {
        val selection = selectionRange()
        if (!Screen.hasShiftDown() && selection != null) {
        ...
        if (Screen.hasShiftDown() && selectionAnchor == null) selectionAnchor = cursor
